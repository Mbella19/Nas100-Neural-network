...
input int      BootstrapBars = 12000;       // ~8.3 days of M1 bars
input bool     SendComponents = true;       // Send component symbols (AAPL, MSFT, ...)
input string   ComponentSymbols = "AAPL,MSFT,NVDA,AMZN,GOOG,AVGO";
input int      ComponentBootstrapBars = 500; // ~8.3 hours of M1 bars for components
...
string BuildJsonPayload()
{
   ...
   // Optional component symbols for cross-asset features/attention
   if(SendComponents) {
      json += "\"components\":{";
      string symbols = ComponentSymbols;
      string parts[];
      ...
      for(int i=0; i<n; i++) {
         ...
         if(sym == "") continue;
         int compBars = bootstrap ? ComponentBootstrapBars : 1;
         if(!first) json += ",";
         json += "\"" + sym + "\":" + GetRatesJson(sym, PERIOD_M1, compBars, 1);
         first = false;
      }
      json += "},";
   }
